...
def main():
    ...
    train_data = DataLoader(dataset=train_data, batch_size=cfg.batch_size, shuffle=False, collate_fn=custom_collate,
                            pin_memory=True)
    ...
    f = open("log_train_torch.txt", "a")
    all_step, all_g_loss, all_db_loss, all_df_loss = [], [], [], []
    ...
    for step in tqdm(range(cfg.max_iter)):
        ...
        if (step + 1) % 2 == 0:
            ...
            all_step.append(step + 1)
            all_g_loss.append(g_loss.item())
            all_db_loss.append(db_loss.item())
            all_df_loss.append(df_loss.item())
    ...
    f.close()
    fig = plt.figure()
    plot(all_g_loss, all_step, 'g_loss')
    plot(all_db_loss, all_step, 'db_loss')
    plot(all_df_loss, all_step, 'df_loss')
